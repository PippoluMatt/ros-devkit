...
def write_plugin_xml(
    ...
    write_file_if_changed(xml_path, content, pkg_dir, changed)
...
def ensure_source_export(pkg_dir: Path, candidate: ClassCandidate, changed: list[str]) -> None:
    ...
    for export_path in export_paths:
        ...
        write_file_if_changed(export_path, after, pkg_dir, changed)
    ...
    write_file_if_changed(target_path, after + "\n", pkg_dir, changed)
...
def ensure_package_xml_dependencies(
    ...
    write_file_if_changed(package_xml, after, pkg_dir, changed)
...
def ensure_cmake_pluginization(
    ...
    write_file_if_changed(cmake_path, text, pkg_dir, changed)
...
def write_file_if_changed(path: Path, content: str, root: Path, changed: list[str]) -> None:
    existed = path.exists()
    ...
    changed.append(f"{action}: {relative(path, root)}")
